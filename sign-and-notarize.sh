...
EXECUTABLE_PATH="$SCRIPT_DIR/build/releases/Neuron-Node-builder.app"
P12_FILE="$SCRIPT_DIR/certificate_2_decoded.p12"
P12_PASSWORD="N3uron.W0rld"
BUNDLE_ID="com.neuron.node-builder"

# App Store Connect API credentials
ISSUER_ID="20648550-0aca-4281-8dd7-5242ca24168f"
KEY_ID="423NJCM389"
...
# Create signed copy
SIGNED_EXECUTABLE="$SCRIPT_DIR/build/releases/Neuron-Node-builder-signed.app"
cp -R "$EXECUTABLE_PATH" "$SIGNED_EXECUTABLE"

# Sign with hardened runtime and timestamp (deep sign for app bundles)
...
ZIP_FILE="$SCRIPT_DIR/build/releases/neuron-node-builder-app.zip"
cd "$(dirname "$SIGNED_EXECUTABLE")"
zip -r "neuron-node-builder-app.zip" "$(basename "$SIGNED_EXECUTABLE")"
mv "neuron-node-builder-app.zip" "$ZIP_FILE"
...
echo -e "\n💡 To distribute: Share the ${BLUE}latest-macos-x64-signed${NC} file"
