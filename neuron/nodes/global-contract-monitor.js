...
async function initializeContractsFromRegistry() {
  try {
    ...
    const filteredContracts = registryContracts.filter(
      (c) =>
    ...
    filteredContracts.forEach((contract) => {
      ...
      contractLoadingStates[contract.name] =
        contractLoadingStates[contract.name] || false;
      ...
      if (!fs.existsSync(cacheFilePath)) {
        ...
        if (fs.existsSync(sampleCachePath)) {
          cacheSampleData = JSON.parse(
            fs.readFileSync(sampleCachePath, "utf-8")
          );
        }

        fs.writeFileSync(
          cacheFilePath,
          JSON.stringify(cacheSampleData, null, 2)
        );
      }
    ...
    console.log(
      `[GlobalContractMonitor] Initialized ${contracts.length} contracts from registry:`,
      contracts
    );
    return true;
  } catch (error) {
    console.error(
      "[GlobalContractMonitor] Failed to initialize contracts from registry:",
      error
    );

    // Fallback to hardcoded contracts if registry fails
    console.warn(
      "[GlobalContractMonitor] Falling back to environment variable contracts"
    );
