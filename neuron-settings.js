...
module.exports = {
  ...
  httpAdminMiddleware: function (req, res, next) {
    ...
    // DEBUG: Log mode detection on first request to editor
    if (req.path === "/" && !global.__visualModeLogged) {
      global.__visualModeLogged = true;
      console.log("🎭 Visual Test Mode Check:", {
        VISUAL_TEST_MODE: process.env.VISUAL_TEST_MODE,
        isVisualTestMode,
        CI: process.env.CI,
        path: req.path,
      });
    ...
    if (!isVisualTestMode) {
      ...
      if (
        ...
        console.log(
          "🔄 Redirecting to setup page due to missing credentials:",
          missingVars
        );
        return res.redirect("/neuron/pages/setup.html");
      }
    } else {
      // Visual test mode: log and skip all redirects
      if (req.path === "/") {
        console.log(
          "🎭 Visual test mode active - serving editor without credential checks"
        );
      }
    }
    ...
    if (
      req.method === "POST" &&
      req.path === "/neuron/setup/save-credentials"
    ) {
      let body = "";
      req.on("data", (chunk) => {
        body += chunk.toString();
      ...
      req.on("end", () => {
        try {
          ...
          console.log("📥 Received credentials:", {
            HEDERA_OPERATOR_ID: credentials.HEDERA_OPERATOR_ID
              ? "✅ Set"
              : "❌ Empty",
            HEDERA_OPERATOR_KEY: credentials.HEDERA_OPERATOR_KEY
              ? "✅ Set"
              : "❌ Empty",
          });
          ...
          const envPath =
            require("./neuron/services/NeuronEnvironment").getPath();
          let envContent = "";
          ...
          const updates = [
            {
              ...
              value: `HEDERA_OPERATOR_ID=${credentials.HEDERA_OPERATOR_ID}`,
            },
            {
              key: "HEDERA_OPERATOR_KEY",
              value: `HEDERA_OPERATOR_KEY=${credentials.HEDERA_OPERATOR_KEY}`,
            },
          ];
          ...
          console.log(
            "  HEDERA_OPERATOR_KEY:",
            process.env.HEDERA_OPERATOR_KEY ? "✅ Set" : "❌ Missing"
          );
          ...
          console.log(
            " | \\ | | ___ _   _ _ __ ___  _ __   | __ ) _   _(_) | __| | ___ _ __ "
          );
          console.log(
            " |  \\| |/ _ \\ | | | '__/ _ \\| '_ \\  |  _ \\| | | | | |/ _` |/ _ \\ '__|"
          );
          ...
          console.log(
            " |_| \\_|\\___|\\__,_|_|  \\___/|_| |_| |____/ \\__,_|_|_|\\__,_|\\___|_|   "
          );
          console.log(
            "                                                                     "
          );
    ...
    // Skip update checks entirely in visual test mode
    if (isVisualTestMode) {
      return next();
    }
    ...
    updateService.checkForUpdates(requestUrl.searchParams).then((response) => {
      if (response.type === "redirect") {
        console.log(
          `[middleware] Update available, redirecting to: ${response.url}`
        );
  ...
  httpStatic: [
    ...
    ...(process.env.VISUAL_TEST_MODE === "1"
      ? [
          {
            path: path.resolve(__dirname, "tests/visual/fixtures/"),
            root: "/visual-fixtures/",
          },
    ...
    page: {
      ...
      css: [
        ...
        require("path").resolve(
          __dirname,
          "neuron/theme/chat-widget-layout.css"
        ),
      ],
      scripts: [
        ...
        require("path").resolve(
          __dirname,
          "neuron/theme/chat-server-config.js"
        ),
        require("path").resolve(
          __dirname,
          "neuron/theme/neuron-chat-widget.js"
        ),
        require("path").resolve(
          __dirname,
          "neuron/theme/template-browser-service.js"
        ),
        require("path").resolve(
          __dirname,
          "neuron/theme/template-browser-dialog.js"
        ),
        require("path").resolve(
          __dirname,
          "neuron/theme/template-browser-button-simple.js"
        ),
      ],
    },
